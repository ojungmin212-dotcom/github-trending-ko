...
각 카드 오른쪽 위의 **☆**를 누르면 **★**로 바뀌고, 기간 탭 오른쪽의 **★ 관심 레포** 탭에 모입니다.

- 관심 탭에서는 순위 자리에 "오늘 #3"처럼 **현재 순위**를 보여주고, 트렌딩에서 빠진 레포는 "순위 밖"으로 표시한 채 목록에 남깁니다. 저장해 둔 설명·스타 정보를 그대로 보여주며, 다시 순위에 오르면 최신 값으로 갱신합니다.
- 필터·검색·정렬과 📖 가이드는 관심 탭에서도 똑같이 동작합니다 (정렬 기본값은 "최근 추가순").
- **저장 위치:** 브라우저의 `localStorage` (키 `gtk-favs-v1`). 서버가 없는 정적 사이트라 **기기·브라우저마다 따로** 저장되고, 사생활 보호 모드나 사이트 데이터 삭제 시 사라집니다.
- **다른 기기로 옮기기:** 관심 탭의 **🔗 다른 기기로 옮기기**를 누르면 `…/?fav=owner/repo,owner/repo` 링크가 복사됩니다. 휴대폰 등에서 이 링크를 열면 그 기기의 관심 목록에 **합쳐지고**(기존 항목은 유지), 주소창의 `?fav=` 부분은 자동으로 지워집니다. 형식에 맞지 않는 이름은 무시합니다.
- 같은 브라우저의 다른 탭에서 바꾼 내용은 열려 있는 탭에도 바로 반영됩니다.
